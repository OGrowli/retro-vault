...
export function ScrapeSettings({ systems, onBack }: Props) {
  const [devId, setDevId] = useState('')
  const [devPassword, setDevPassword] = useState('')
  const [username, setUsername] = useState('')
  const [password, setPassword] = useState('')
  const [selectedSystem, setSelectedSystem] = useState(systems[0] ?? '')
  const [focused, setFocused] = useState<FocusItem>('dev-id')
  ...
  // Keep the gamepad-focused control in view as the user navigates.
  useEffect(() => {
